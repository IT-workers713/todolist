...
/**
 * TODOLIST.TextIO provides a set of static methods for reading and writing text.  By default, it reads
 ...
 * behavior; to handle this case, TODOLIST.TextIO will give up after 10 consecutive illegal inputs and
 ...
 * <p>This class requires Java 5.0 or higher. (A previous version of TODOLIST.TextIO required only Java 1.1;
 ...
 * <p>TODOLIST.TextIO must be imported from package textio to be used.  (Previous versions were in the
...
public class TextIO {
    ...
    /* Modified May 2018 to move TODOLIST.TextIO.java into package textio.  This class is otherwise
     * identical to the version in the default package.
     */

    /* Modified November 2007 to empty the TODOLIST.TextIO input buffer when switching from one
    ...
    /**
     * If TODOLIST.TextIO is currently reading from a file, then the return value is the name of the file.
    ...
    /**
     * If TODOLIST.TextIO is currently writing to a file, then the return value is the name of the file.
    ...
    public static void putf(String format, Object... items) {
        if (format == null)
            throw new IllegalArgumentException("Null format string in TODOLIST.TextIO.putf() method.");
        try {
            out.printf(format,items);
        }
        catch (IllegalFormatException e) {
            throw new IllegalArgumentException("Illegal format string in TODOLIST.TextIO.putf() method.");
        }
        out.flush();
        if (out.checkError())
            outputError("Error while writing output.");
     ...
     * end-of-file character (specified by the constant TODOLIST.TextIO.EOF).An end-of-line is always returned
    ...
    private static void outputError(String message) {  // Report an error on output.
        if (writingStandardOutput) {
            System.err.println("Error occurred in TODOLIST.TextIO while writing to standard output!!");
            outputErrorCount++;
